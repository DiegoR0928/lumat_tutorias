...
class DocenteFirmarSeminarioTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user_tutor = User.objects.create_user(username='tutor_u', password='pwd')
        self.user_tutor.groups.add(self.grupo_docente)
        self.tutor = Docente.objects.create(user=self.user_tutor, nombre="Carlos", apellido_paterno="Lopez", correo="t@uaz.mx")

        self.user_m1 = User.objects.create_user(username='m1_u', password='pwd')
        self.user_m1.groups.add(self.grupo_docente)
        self.m1 = Docente.objects.create(user=self.user_m1, nombre="Maria", apellido_paterno="Gomez", correo="m1@uaz.mx")

        self.user_m2 = User.objects.create_user(username='m2_u', password='pwd')
        self.user_m2.groups.add(self.grupo_docente)
        self.m2 = Docente.objects.create(user=self.user_m2, nombre="Jose", apellido_paterno="Sanchez", correo="m2@uaz.mx")

        # 2. Configurar Alumno e infraestructura del Comité/Seminario
        self.user_alumno = User.objects.create_user(username='alumno_u', password='pwd')
        self.alumno = Alumno.objects.create(user=self.user_alumno, matricula="20220001", nombre="Luis")
        
        self.comite = Comite.objects.create(tutor=self.tutor, miembro1=self.m1, miembro2=self.m2)
        ...
        self.url_firmar = reverse('lumat_app:docente_firmar_seminario', kwargs={'seminario_id': self.seminario.id})
    ...
    def test_docente_ya_firmado_bloquea_post_y_redirige(self):
        ...
        with patch.object(FormularioComite, 'save', return_value=None):
            response = self.client.post(self.url_firmar, data=payload, follow=True)
    ...
    def test_formulario_firma_invalido_agrega_mensaje_error(self):
        ...
        payload = {'calificacion': 15.0, 'confirmar_firma': True} # 15.0 está fuera del rango [0, 10]
        with patch.object(FormularioComite, 'save', return_value=None):
            response = self.client.post(self.url_firmar, data=payload, follow=True)

        self.assertEqual(response.status_code, 200)
        mensajes = list(get_messages(response.wsgi_request))
        self.assertEqual(str(mensajes[0]), 'Datos inválidos. Verifica la calificación.')
