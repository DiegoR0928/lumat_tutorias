...
class TestCrearComiteAdminView(TestCase):
    ...
    def test_crear_comite_usa_template_correcto(self):
        """Verifica que se use el template de creación del panel de administración."""
        self.client.login(username='admin', password='testpass123')
        response = self.client.get(self.url_crear_comite)
        # El admin de Django usa 'admin/change_form.html' para las vistas de "add"
        self.assertTemplateUsed(response, 'admin/change_form.html')

    # --- Redirecciones ---

    def test_usuario_normal_redirige_a_admin_login(self):
        """Un usuario sin permisos debe ser redirigido a la página de login del admin."""
    ...
    def test_creacion_exitosa_retorna_302(self):
        """Al crear un comité correctamente, el admin hace un redirect (302) al listado."""
        self.client.login(username='admin', password='testpass123')
    ...
    def test_docentes_repetidos_muestra_mensaje_error(self):
        """Si se envía el mismo docente, la vista no redirige (200) y muestra el error de validación."""
        self.client.login(username='admin', password='testpass123')
        ...
        self.assertContains(response, 'Los tres docentes del comité deben ser distintos.')
