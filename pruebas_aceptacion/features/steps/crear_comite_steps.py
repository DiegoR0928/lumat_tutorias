...
def step_impl(context):
    ...
    dropdown_miembro1 = Select(
        context.driver.find_element(By.NAME, 'miembro1'))
    dropdown_miembro1.select_by_visible_text('Diego Gomez')

    dropdown_miembro2 = Select(
        context.driver.find_element(By.NAME, 'miembro2'))
...
def step_impl(context, nombre):
    # Buscamos el dropdown de tutor y seleccionamos al docente+
    # por texto parcial
...
@then(
    u'el sistema debería resaltar los campos obligatorios '
    u'y no guardar el comité'
)
def step_impl(context):
    # 1. Verificamos que NO nos hayamos movido de la página de creación.
    assert "add" in context.driver.current_url, (
        "El sistema redirigió, lo que indica que el comité "
        "se guardó por error."
    )
    ...
    try:
        ...
        assert is_required is not None, (
            "El campo no tiene el atributo 'required' ni se mostró "
            "error de Django."
        )
